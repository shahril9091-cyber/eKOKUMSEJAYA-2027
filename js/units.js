...
const Units = (() => {
  ...
  async function render() {
    ...
    if (!loaded) {
      ...
      [units, teachers, allStudents] = await Promise.all([Api.getUnits(), Api.getTeachers(), Api.getStudents()]);
      loaded = true;
    }
    root.innerHTML = `
  ...
  function renderRows() {
    ...
    document.getElementById("unit-tbody").innerHTML = filtered.length ? filtered.map((u) => `
      <tr>
        ...
        <td class="row-actions">
          <button class="btn btn-icon btn-outline" data-id="${u.unit_id}" data-act="members" title="Uruskan Ahli"><i class="fa-solid fa-user-plus"></i></button>
    ...
    document.getElementById("unit-cardlist").innerHTML = filtered.length ? filtered.map((u) => `
      <div class="cr-item">
        ...
        <div class="cr-actions">
          <button class="btn btn-sm btn-outline" data-id="${u.unit_id}" data-act="members">Ahli</button>
    ...
    document.querySelectorAll("#sa-panel-unit [data-act]").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        else if (btn.dataset.act === "members") openMembersModal(rec);
  ...
  // ---- Uruskan Ahli (pengurusan keahlian pukal — daftar ramai murid ke
  // satu unit sekali gus, berbanding kena buka borang setiap murid) ----

  async function openMembersModal(unit) {
    UI.openModal({
      title: `Uruskan Ahli — ${unit.unit_name}`,
      size: "lg",
      bodyHtml: `<div class="loading-inline"><div class="spinner spinner-dark"></div>Memuatkan senarai murid...</div>`,
      footerHtml: `<button class="btn btn-outline" onclick="UI.closeModal()">Tutup</button><button class="btn btn-primary" id="unit-members-save-btn">Simpan</button>`
    });
    document.getElementById("unit-members-save-btn").disabled = true;

    let currentMemberIds = [];
    try {
      const members = await Api.getMembers({ unit_id: unit.unit_id });
      currentMemberIds = members.filter((m) => m.status !== "DELETED").map((m) => m.student_id);
    } catch (e) { /* borang masih boleh dibuka, cuma tiada pra-pilihan */ }

    const modalBody = document.querySelector("#modal-overlay .modal-body");
    modalBody.innerHTML = `
      <p class="hint" style="margin-bottom:10px;">Tandakan murid yang menjadi ahli unit ini. Boleh cari nama untuk senarai panjang.</p>
      <div class="filter-search" style="margin-bottom:10px;">
        <i class="fa-solid fa-magnifying-glass"></i>
        <input type="text" class="input" id="unit-members-search" placeholder="Cari nama murid...">
      </div>
      <div style="margin-bottom:10px; display:flex; gap:8px;">
        <button type="button" class="btn btn-sm btn-outline" id="unit-members-select-all">Pilih Semua (Ditapis)</button>
        <button type="button" class="btn btn-sm btn-outline" id="unit-members-clear-all">Nyahpilih Semua</button>
        <span class="hint" id="unit-members-count" style="margin-left:auto; align-self:center;"></span>
      </div>
      <div id="unit-members-list" style="max-height:360px; overflow-y:auto; border:1px solid var(--border); border-radius:var(--radius-sm); padding:8px 12px;"></div>
    `;

    renderMembersChecklist_(allStudents, currentMemberIds);
    updateMembersCount_();

    document.getElementById("unit-members-search").addEventListener("input", (e) => {
      const term = e.target.value.toLowerCase();
      const filtered = allStudents.filter((s) => s.name.toLowerCase().includes(term));
      const checkedNow = getCheckedMemberIds_();
      renderMembersChecklist_(filtered, checkedNow);
    });

    document.getElementById("unit-members-select-all").addEventListener("click", () => {
      document.querySelectorAll("#unit-members-list .unit-member-check").forEach((cb) => (cb.checked = true));
      updateMembersCount_();
    });
    document.getElementById("unit-members-clear-all").addEventListener("click", () => {
      document.querySelectorAll("#unit-members-list .unit-member-check").forEach((cb) => (cb.checked = false));
      updateMembersCount_();
    });

    document.getElementById("unit-members-save-btn").disabled = false;
    document.getElementById("unit-members-save-btn").addEventListener("click", async () => {
      const studentIds = getCheckedMemberIds_();
      const btn = document.getElementById("unit-members-save-btn");
      btn.disabled = true;
      btn.innerHTML = `<div class="spinner"></div> Menyimpan...`;
      try {
        await Api.setUnitMembers(unit.unit_id, studentIds);
        UI.closeModal();
        UI.toast(`${studentIds.length} murid ditetapkan sebagai ahli ${unit.unit_name}.`, "success");
        loaded = false;
        render();
      } catch (err) {
        UI.toast("Gagal menyimpan keahlian: " + err.message, "error");
        btn.disabled = false;
  ...
  // Nota: senarai yang dipaparkan (ditapis carian) boleh berubah, tetapi
  // pilihan murid yang TIDAK kelihatan (kerana ditapis) mesti dikekalkan.
  // Jadi kita simpan status tercentang setiap murid dalam Set berasingan
  // daripada senarai checkbox yang sedang dipaparkan.
  let membersCheckedSet_ = new Set();

  function renderMembersChecklist_(studentsToShow, checkedIds) {
    membersCheckedSet_ = new Set(checkedIds);
    const listEl = document.getElementById("unit-members-list");
    if (!studentsToShow.length) {
      listEl.innerHTML = `<span class="hint">Tiada murid sepadan carian.</span>`;
      return;
    }
    listEl.innerHTML = studentsToShow.map((s) => `
      <label style="display:flex; align-items:center; gap:8px; padding:5px 0; font-size:13px; cursor:pointer;">
        <input type="checkbox" class="unit-member-check" value="${s.student_id}" ${membersCheckedSet_.has(s.student_id) ? "checked" : ""}>
        ${s.name} <span style="color:var(--text-400); font-size:11.5px;">(${s.class})</span>
      </label>
    `).join("");
    listEl.querySelectorAll(".unit-member-check").forEach((cb) => {
      cb.addEventListener("change", () => {
        if (cb.checked) membersCheckedSet_.add(cb.value);
        else membersCheckedSet_.delete(cb.value);
        updateMembersCount_();
      });
    });
  }

  function getCheckedMemberIds_() {
    // Gabungkan checkbox yang SEDANG dipaparkan dengan set tercentang yang
    // dikekalkan (untuk murid yang ditapis keluar semasa carian).
    document.querySelectorAll("#unit-members-list .unit-member-check").forEach((cb) => {
      if (cb.checked) membersCheckedSet_.add(cb.value);
      else membersCheckedSet_.delete(cb.value);
    });
    return Array.from(membersCheckedSet_);
  }

  function updateMembersCount_() {
    const countEl = document.getElementById("unit-members-count");
    if (countEl) countEl.textContent = `${getCheckedMemberIds_().length} murid dipilih`;
  }

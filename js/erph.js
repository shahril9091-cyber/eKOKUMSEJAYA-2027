...
const Erph = (() => {
  ...
  function renderRows() {
    ...
    cardlist.innerHTML = records.map((r) => `
      <div class="cr-item">
        ...
        <div class="cr-line"><span>Guru</span><span>${teacherName_(r.teacher_ids)}</span></div>
  ...
  async function handlePdf(rec) {
    ...
    try {
      await PdfGenerator.generateErphPdf({ ...rec, unit: unitName_(rec.unit_id), teacher: teacherName_(rec.teacher_ids) });
  ...
  async function handleSave(rec, isEdit) {
    ...
    const payload = {
      ...
      teacher_ids: Array.from(document.querySelectorAll(".erph-teacher-check:checked")).map((el) => el.value),

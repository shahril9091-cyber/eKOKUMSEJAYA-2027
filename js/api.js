...
const Api = (() => {
  ...
  const DUMMY = {
    ...
    reports: [
      { report_id: "R001", date: "2026-08-16", minggu: "1", unit_id: "U001", category: "UNIT_BERUNIFORM", activity_title: "Latihan Ikatan & Simpulan", teacher_ids: "T003", present: 38, total_students: 42, status: "LENGKAP" },
      { report_id: "R002", date: "2026-08-16", minggu: "1", unit_id: "U005", category: "KELAB_PERSATUAN", activity_title: "Eksperimen Roket Air", teacher_ids: "T003", present: 30, total_students: 33, status: "LENGKAP" },
      { report_id: "R003", date: "2026-08-17", minggu: "1", unit_id: "U006", category: "SUKAN_PERMAINAN", activity_title: "Kem Latihan Kemahiran Asas", teacher_ids: "T003", present: 25, total_students: 28, status: "LENGKAP" },
      { report_id: "R004", date: "2026-08-23", minggu: "2", unit_id: "U007", category: "SUKAN_PERMAINAN", activity_title: "Perlawanan Persahabatan", teacher_ids: "T004", present: 22, total_students: 24, status: "PENDING" },
      { report_id: "R005", date: "2026-08-23", minggu: "2", unit_id: "U003", category: "UNIT_BERUNIFORM", activity_title: "Tazkirah & Gotong-Royong", teacher_ids: "T002", present: 28, total_students: 30, status: "LENGKAP" }
    ],

    erph: [
      { erph_id: "E001", date: "2026-08-16", minggu: "1", unit_id: "U001", topic: "Latihan Ikatan & Simpulan", teacher_ids: "T003", status: "LENGKAP" },
      { erph_id: "E002", date: "2026-08-23", minggu: "2", unit_id: "U001", topic: "Navigasi Asas & Peta Pandu Arah", teacher_ids: "T003", status: "DRAF" }
    ],
  ...
  function dummyResponse(action, payload) {
    switch (action) {
      ...
      case "setUnitMembers": {
        DUMMY.members = (DUMMY.members || []).filter((m) => m.unit_id !== payload.unit_id);
        (payload.student_ids || []).forEach((studentId) => {
          DUMMY.members.push({ member_id: nextDummyId_(DUMMY.members, "member_id", "M"), student_id: studentId, unit_id: payload.unit_id, status: "AKTIF" });
        });
        return { unit_id: payload.unit_id, student_ids: payload.student_ids };
      }
      ...
      // ---- eRPH ----
      case "saveERPH": {
        const teacherIdsStr = Array.isArray(payload.teacher_ids) ? payload.teacher_ids.join(",") : (payload.teacher_ids || "");
        const normalized = { ...payload, teacher_ids: teacherIdsStr };
        if (payload.erph_id) {
          Object.assign(DUMMY.erph.find((e) => e.erph_id === payload.erph_id) || {}, normalized);
          return { erph_id: payload.erph_id };
        }
        const id = nextDummyId_(DUMMY.erph, "erph_id", "E");
        const obj = { erph_id: id, status: "DRAF", ...normalized };
        DUMMY.erph.push(obj);
        return obj;
      }
      case "deleteERPH":
      ...
      case "saveActivityReport": {
        const teacherIdsStr = Array.isArray(payload.teacher_ids) ? payload.teacher_ids.join(",") : (payload.teacher_ids || "");
        const total = Number(payload.total_students) || 0;
        const present = Number(payload.present) || 0;
        const normalized = { ...payload, teacher_ids: teacherIdsStr, absent: total - present, attendance_percentage: total > 0 ? Math.round((present / total) * 100) : 0 };
        if (payload.report_id) {
          Object.assign(DUMMY.reports.find((r) => r.report_id === payload.report_id) || {}, normalized);
          return { report_id: payload.report_id };
        }
        const id = nextDummyId_(DUMMY.reports, "report_id", "R");
        const obj = { report_id: id, status: "LENGKAP", ...normalized };
      ...
      case "uploadImagesBulk":
        return { uploaded: (payload.images || []).map((img, i) => ({ image_id: "IMG" + Date.now() + i, image_url: "", caption: img.caption || "" })) };
  ...
  return {
    ...
    setUnitMembers: (unitId, studentIds) => request("setUnitMembers", { unit_id: unitId, student_ids: studentIds }, "POST"),
    ...
    uploadImagesBulk: (data) => request("uploadImagesBulk", data, "POST")
  };
})();

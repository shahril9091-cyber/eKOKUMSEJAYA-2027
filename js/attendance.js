...
const Attendance = (() => {
  ...
  // Tahun ditentukan daripada angka pertama dalam medan Kelas yang
  // Superadmin daftarkan (cth "3 Al Biruni" -> Tahun 3). Ini memastikan
  // penentuan Tahun sentiasa ikut pendaftaran sebenar, bukan tekaan lain.
  function tahunOf_(kelas) {
    const match = String(kelas || "").match(/^(\d+)/);
    return match ? Number(match[1]) : null;
  }

  function groupByTahun_(list) {
    const groups = {};
    list.forEach((s) => {
      const tahun = tahunOf_(s.class);
      const key = tahun !== null ? tahun : "lain";
    ...
    return sortedKeys.map((key) => ({
      label: key === "lain" ? "Tahun Tidak Diketahui" : `Tahun ${key}`,
      students: groups[key]
    }));
  }
  ...
  function renderList() {
    ...
    const groups = groupByTahun_(students);
    let runningIndex = 0;

    tbody.innerHTML = groups.map((g) => `
      <tr class="tahun-group-row">
        <td colspan="5" style="background:var(--surface-muted); font-weight:700; font-size:12px; color:var(--navy-800); padding:8px 14px;">
          ${g.label} <span style="font-weight:400; color:var(--text-400);">(${g.students.length} murid)</span>
        </td>
      </tr>
      ${g.students.map((s) => {
        runningIndex += 1;
    ...
    let cardRunningIndex = 0;
    cardlist.innerHTML = groups.map((g) => `
      <div class="tahun-group-heading" style="font-weight:700; font-size:12px; color:var(--navy-800); background:var(--surface-muted); padding:8px 14px; border-radius:var(--radius-sm); margin-bottom:8px;">
        ${g.label} <span style="font-weight:400; color:var(--text-400);">(${g.students.length} murid)</span>
      </div>
      ${g.students.map((s) => {
        cardRunningIndex += 1;
        return `
        <div class="cr-item">
          <div class="cr-top"><span class="cr-title">${cardRunningIndex}. ${s.name}</span></div>

...
const Reports = (() => {
  ...
  async function openForm(rec = {}, readOnly = false) {
    const isEdit = !!rec.report_id;
    selectedImages = [null, null, null, null];

    // Bila mod "Lihat" (baca sahaja), tarik gambar SEBENAR yang telah
    // dimuat naik — sebelum ini borang baca-sahaja hanya papar placeholder
    // statik "Landscape sahaja" walaupun gambar sudah wujud di Drive.
    let existingImages = [];
    if (readOnly && rec.report_id) {
      try {
        existingImages = await Api.getImagesByReport(rec.report_id);
      } catch (e) { /* borang masih boleh dipaparkan tanpa gambar */ }
    }
    ...
    const imageSlots = CAPTIONS.map((cap, i) => {
      ...
      <div class="image-slot" id="img-slot-${i}" ${readOnly ? "" : `onclick="document.getElementById('img-input-${i}').click()"`} style="${readOnly ? "" : "cursor:pointer;"}">
        ...
        ${readOnly && existing ? `<img src="${existing.image_url}" style="width:100%; height:100%; object-fit:cover;">` : `
          <i class="fa-solid fa-image fa-lg" id="img-icon-${i}"></i>
          <span id="img-hint-${i}">${readOnly ? "Tiada gambar" : "Landscape sahaja"}</span>
        `}
        <input type="text" id="img-caption-${i}" placeholder="Kapsyen" value="${(existing && existing.caption) || cap}" ${readOnly ? "disabled" : ""} onclick="event.stopPropagation()">

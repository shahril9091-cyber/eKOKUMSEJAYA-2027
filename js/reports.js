...
const Reports = (() => {
      ...
      bodyHtml: `
        <form id="rep-form">
          ...
          <div class="form-grid cols-3">
            ...
            <div class="form-field"><label>Guru Penasihat</label><select class="input" id="rep-teacher" ${readOnly ? "disabled" : ""}><option value="">Pilih Guru</option>${teacherOptions}</select></div>
            <div class="form-field"><label>Bilangan Murid</label><input type="number" class="input" id="rep-total" value="${rec.total || ""}" ${readOnly ? "disabled" : ""}></div>
          ...
          <div class="form-field"><label>Tajuk Aktiviti</label><input type="text" class="input" id="rep-title" value="${rec.title || ""}" ${readOnly ? "disabled" : ""}></div>
  ...
  async function handleSave(rec, isEdit) {
    ...
    const payload = {
      ...
      teacher_id: document.getElementById("rep-teacher").value,
    ...
    try {
      ...
      // Muat naik gambar yang dipilih (jika ada) — dijalankan selepas laporan
      // disimpan supaya setiap imej boleh dikaitkan dengan report_id yang sah.
      const uploads = selectedImages
        .map((img, i) => (img ? { ...img, index: i } : null))
        .filter(Boolean);

      for (const img of uploads) {
        const base64 = await fileToBase64_(img.file);
